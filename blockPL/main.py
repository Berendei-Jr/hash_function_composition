...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QGraphicsView, 
                             ...
                             QLabel, QComboBox, QMessageBox, QToolBar, QLineEdit)
...
class Connection(QGraphicsPathItem):
    ...
    def updatePath(self):
        ...
        path.cubicTo(start_pos.x() + dx, start_pos.y(),
...
class Node(QGraphicsItem):
    NODE_TYPES = {
        ...
        #"": {"inputs": 1, "color": QColor(175, 238, 238)},   # Pale Turquoise
        #"": {"inputs": 1, "color": QColor(255, 192, 203)}, # Pink
        #"": {"inputs": 1, "color": QColor(230, 230, 250)},     # Lavender
        #"": {"inputs": 1, "color": QColor(144, 238, 144)}, # Light Green
        #"": {"inputs": 2, "color": QColor(255, 165, 0)},     # Orange
        #"": {"inputs": 2, "color": QColor(186, 85, 211)},     # Medium Orchid
        #"": {"inputs": 2, "color": QColor(60, 179, 113)},     # Medium Sea Green
        #"": {"inputs": 2, "color": QColor(255, 99, 71)},      # Tomato
        #"": {"inputs": 2, "color": QColor(123, 104, 238)},    # Medium Slate Blue
        #"": {"inputs": 0, "color": QColor(106, 90, 205)}    # Slate Blue
    }
...
class GraphicsScene(QGraphicsScene):
    ...
    def mouseReleaseEvent(self, event):
        if self.temp_connection:
            ...
            if (isinstance(item, Port) and 
                item != self.start_connection_port and
                item.is_output != self.start_connection_port.is_output):
...
class MainWindow(QMainWindow):
    ...
    def addNode(self, node_type = None):
        ...
        node.setPos(view_center.x() - node.width/2 + randint(-10, 10)*node.width/10, view_center.y() - node.height/2 + randint(-10, 10)*node.height/10)
...
clEnqueueReadBuffer(queue, buf_v{vars[0]}_size, CL_TRUE, 0,
    ...
    def executeProgram(self):
        ...
        QMessageBox.information(self, "Выполнение", 
        ...
        for node_id in range(len(nodes)):
            if not visited[node_id]:
                if not visit(node_id):
                    print("Обнаружен цикл в графе! Невозможно определить порядок выполнения.")
                    return None
